...
def create_user(
    ...
    *,
    ...
    # Immediately login to get a bearer token for subsequent requests.
    login = client.post("/auth/login", json={"email": email, "password": password})
    assert login.status_code == 200
    user["_token"] = login.json()["access_token"]
    return user


def auth_headers(user):
    return {"Authorization": f"Bearer {user['_token']}"}


def create_workout(client, user, *, activity_id="123456789"):
    response = client.post(
...
def create_running_plan(client, user, *, planned_at=None, distance_km=10.0, speed_kph=12.0):
...
def test_create_and_get_user(client):
    ...
    get_response = client.get(f"/users/{user['id']}", headers=auth_headers(user))
...
def test_created_at_timestamp_has_no_fractional_seconds(client):
    ...
    user = create_user(client)
    workout = create_workout(client, user)
    plan = client.post(
...
def test_get_user_forbidden_for_other_user(client):
    user_a = create_user(client, email="a@peak.com")
    user_b = create_user(client, email="b@peak.com")

    response = client.get(f"/users/{user_a['id']}", headers=auth_headers(user_b))

    assert response.status_code == 403


def test_update_user_forbidden_for_other_user(client):
    user_a = create_user(client, email="a@peak.com")
    user_b = create_user(client, email="b@peak.com")
...
def test_create_get_and_list_workouts(client):
    ...
    get_response = client.get(f"/workouts/{workout['id']}", headers=auth_headers(user))
    list_response = client.get(f"/users/{user['id']}/workouts", headers=auth_headers(user))
...
# ---------------------------------------------------------------------------
# Workouts — auth / error cases
# ---------------------------------------------------------------------------


def test_duplicate_strava_activity_returns_conflict(client):
    user = create_user(client)
    create_workout(client, user, activity_id="dup-1")
...
def test_get_unknown_workout_returns_404(client):
    ...
    response = client.get("/workouts/does-not-exist", headers=auth_headers(user))

    assert response.status_code == 404


def test_get_workout_from_other_user_returns_403(client):
    """GET /workouts/{id} now enforces ownership — 403 for cross-user access."""
    user_a = create_user(client, email="a@peak.com")
    user_b = create_user(client, email="b@peak.com")
    workout = create_workout(client, user_a, activity_id="a-workout")

    response = client.get(f"/workouts/{workout['id']}", headers=auth_headers(user_b))

    assert response.status_code == 403


def test_list_workouts_for_other_user_returns_403(client):
    user_a = create_user(client, email="a@peak.com")
    user_b = create_user(client, email="b@peak.com")

    response = client.get(f"/users/{user_a['id']}/workouts", headers=auth_headers(user_b))
...
def test_create_get_and_list_fueling_plans(client):
    ...
    get_response = client.get(f"/fueling-plans/{plan['id']}", headers=auth_headers(user))
    list_response = client.get(f"/users/{user['id']}/fueling-plans", headers=auth_headers(user))
...
# ---------------------------------------------------------------------------
# Fueling plans — auth / error cases
# ---------------------------------------------------------------------------
...
def test_fueling_plan_rejects_other_users_workout(client):
    ...
    workout = create_workout(client, user_one, activity_id="foreign-workout")
...
def test_get_unknown_fueling_plan_returns_404(client):
    ...
    response = client.get("/fueling-plans/does-not-exist", headers=auth_headers(user))

    assert response.status_code == 404


def test_get_fueling_plan_from_other_user_returns_403(client):
    """GET /fueling-plans/{id} enforces ownership."""
    user_a = create_user(client, email="a@peak.com")
    user_b = create_user(client, email="b@peak.com")
    plan = client.post(
    ...
    response = client.get(f"/fueling-plans/{plan['id']}", headers=auth_headers(user_b))

    assert response.status_code == 403


def test_list_fueling_plans_for_other_user_returns_403(client):
    user_a = create_user(client, email="a@peak.com")
    user_b = create_user(client, email="b@peak.com")

    response = client.get(f"/users/{user_a['id']}/fueling-plans", headers=auth_headers(user_b))
...
def test_create_and_list_running_plans(client):
    user = create_user(client)
    plan = create_running_plan(client, user, distance_km=21.1, speed_kph=11.0)
    ...
    list_resp = client.get(f"/users/{user['id']}/running-plans", headers=auth_headers(user))
...
def test_running_plans_ordered_by_planned_at_ascending(client):
    user = create_user(client)
    create_running_plan(client, user, planned_at="2099-12-01T08:00:00+00:00")
    create_running_plan(client, user, planned_at="2099-06-01T06:00:00+00:00")
    create_running_plan(client, user, planned_at="2099-09-15T07:00:00+00:00")

    plans = client.get(f"/users/{user['id']}/running-plans", headers=auth_headers(user)).json()
...
def test_running_plan_requires_auth(client):
    response = client.post(
        "/users/some-user/running-plans",
...
def test_list_running_plans_for_other_user_returns_403(client):
    user_a = create_user(client, email="a@peak.com")
    user_b = create_user(client, email="b@peak.com")

    response = client.get(f"/users/{user_a['id']}/running-plans", headers=auth_headers(user_b))
...
def test_login_returns_user_and_token_on_valid_credentials(client):
...
def test_bearer_token_grants_access_to_protected_endpoint(client):
    """A valid token from /auth/login must be accepted on GET /auth/me."""
    user = create_user(client, email="me@peak.com", password="secret")

    response = client.get("/auth/me", headers=auth_headers(user))

    assert response.status_code == 200
    assert response.json()["email"] == "me@peak.com"
    assert "password" not in response.json()
...
def test_reset_wipes_and_reinitialises_database(testing_client):
    """After a reset, previously created users are gone and the schema is intact."""
    user = create_user(testing_client, email="before-reset@example.com")

    reset_response = testing_client.post("/test/reset")
    assert reset_response.status_code == 204

    # Old token is now invalid — the user no longer exists in the DB
    me_response = testing_client.get("/auth/me", headers=auth_headers(user))
